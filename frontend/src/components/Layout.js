...
function Layout() {
  ...
  const hideNavRoutes = ['/home', '/login', '/register', '/practice/lesson', '/exercise/practice'];
  ...
  return (
    <div className="bg-bluesea max-w-screen-4xl m-auto h-[100dvh] flex flex-col-reverse md:flex-row">
      ...
      {!hideNavRoutes.includes(location.pathname) && (
        <nav className='border-solid border-t-2 border-gray-500 md:w-4/12 min-w-[300px] md:max-w-[350px] md:p-4 md:border-t-0 md:border-r-2 md:border-black'>
          <NavigationBar />
        </nav>
      )}
